...
class DispoLeadController extends Controller
{
    ...
    public function RepresentativeAllDispoLeads()
    {
        $page = "dispo_leads";
        $Role = Session::get('user_role');
        return view('admin.dispo-leads.dispoleads', compact('page', 'Role'));
    }
...
//             if (sizeof($Check) > 0) {
//               if ($Role == 1) {
//                   return redirect(url('/admin/call-request/add'))->with('error', 'This lead have been submitted by a different user. Please contact your general_manager for more information.');
//               } elseif ($Role == 2) {
...
// }
    ...
    public function LoadRepresentativeAllDispoLeads(Request $request)
    {
        $Role = Session::get('user_role');
        $user_id = Auth::id();
    ...
    public function RepresentativeEditDispoLead($Id)
    {
        $Role = Session::get('user_role');
        $page = "dispo_leads";
        $user_id = Auth::id();
        ...
        if ($Lead[0]->appointment_time != "") {
            $AppointmentTime = Carbon::parse($Lead[0]->appointment_time)->format('m/d/Y - g:i a');
        }

        return view('admin.dispo-leads.edit-dispolead', compact('page', 'Lead', 'SplitOptions', 'products', 'Role', 'AppointmentTime'));
    }
    ...
    public function RepresentativeUpdateLead(Request $request)
    {
        ...
        if ($Affected) {
            ...
            if (sizeof($Check) > 0) {
                ...
                if ($Role == 1) {
                    return redirect(url('/admin/call-requests'))->with('message', 'Dispo Lead has been updated successfully');
                } elseif ($Role == 2) {
                    return redirect(url('/general_manager/call-requests'))->with('message', 'Dispo Lead has been updated successfully');
                } elseif ($Role == 3) {
                    return redirect(url('/confirmationAgent/call-requests'))->with('message', 'Dispo Lead has been updated successfully');
                } elseif ($Role == 4) {
                    return redirect(url('/supervisor/call-requests'))->with('message', 'Dispo Lead has been updated successfully');
                } elseif ($Role == 5) {
                    return redirect(url('/representative/call-requests'))->with('message', 'Dispo Lead has been updated successfully');
                }
            }
        } else {
            DB::rollback();
    ...
    public function AdminConvertDispoLead(Request $request)
    {
        $Role = Session::get('user_role');
        $DispoLeadId = $request['convertDispoLeadId'];
        $CurrentDate = date('Y-m-d');
        $HistoryNote = "Dispo lead is converted into full lead";

        DB::beginTransaction();
        $Affected = DB::table('leads')
            ->where('id', '=', $DispoLeadId)
            ->update([
                'user_id' => Auth::id(),
                'lead_type' => 1,
                'updated_at' => Carbon::now()
        ...
        if ($Affected && $Affected1) {
            DB::commit();
            if ($Role == 1) {
                return redirect(url('/admin/call-requests'))->with('message', 'Dispo lead has been converted to lead successfully');
            } elseif ($Role == 2) {
                return redirect(url('/general_manager/call-requests'))->with('message', 'Dispo lead has been converted to lead successfully');
            } elseif ($Role == 3) {
                return redirect(url('/confirmationAgent/call-requests'))->with('message', 'Dispo lead has been converted to lead successfully');
            } elseif ($Role == 4) {
                return redirect(url('/supervisor/call-requests'))->with('message', 'Dispo lead has been converted to lead successfully');
            } elseif ($Role == 5) {
                return redirect(url('/representative/call-requests'))->with('message', 'Dispo lead has been converted to lead successfully');
            }
        } else {
            DB::rollback();
            if ($Role == 1) {
    ...
    public function LoadCallHistoryNote(Request $request)
    {
        ...
        foreach ($fetch_data as $row => $item) {
            $sub_array = array();
            $sub_array['id'] = $SrNo;
            $sub_array['user_id'] = $item->firstname . " " . $item->lastname;
            $sub_array['history_note'] = $item->history_note;
            $sub_array['created_at'] = Carbon::parse($item->created_at)->format('m/d/Y - g:i a');
            $SrNo++;
            $data[] = $sub_array;
        }
