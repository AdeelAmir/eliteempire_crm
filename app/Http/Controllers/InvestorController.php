...
class InvestorController extends Controller
{
    ...
    /* Admin Section - Start */
    public function AdminAllInvestors()
    {
        $Role = Session::get('user_role');
        $page = "investors";
        return view('admin.investors.investors', compact('page', 'Role'));
    }

    public function AdminAddNewInvestor()
    {
        $Role = Session::get('user_role');
        $page = "investors";
        ...
        return view('admin.investors.add-new-investor', compact('page', 'states', '_states', 'Role'));
    }

    public function AdminInvestorStore(Request $request)
    {
        ...
        if ($check_user_email > 0) {
            if ($UserRole == 1) {
                return redirect(url('/admin/investor/add'))->with('error', 'Error! Email already exists');
            } elseif ($UserRole == 2) {
                return redirect(url('/global_manager/investor/add'))->with('error', 'Error! Email already exists');
            }
        } else {
            DB::beginTransaction();
            ...
            if ($affected && $affected1) {
                DB::commit();
                if ($UserRole == 1) {
                    return redirect(url('/admin/buissness_accounts'))->with('message', 'Investor Account has been added successfully');
                } elseif ($UserRole == 2) {
                    return redirect(url('/global_manager/buissness_accounts'))->with('message', 'Investor Account has been added successfully');
                } elseif ($UserRole == 6) {
                    return redirect(url('/disposition_representative/buissness_accounts'))->with('message', 'Investor Account has been added successfully');
                }
            } else {
                DB::rollback();
                if ($UserRole == 1) {
    ...
    public function AdminDeleteInvestor(Request $request)
    {
        $UserRole = Session::get('user_role');
        $user_id = $request['id'];
        DB::beginTransaction();
        ...
        if ($affected) {
            DB::commit();
            if ($UserRole == 1) {
                return redirect(url('/admin/buissness_accounts'))->with('message', 'Investor account has been deleted successfully');
            } elseif ($UserRole == 2) {
                return redirect(url('/global_manager/buissness_accounts'))->with('message', 'Investor account has been deleted successfully');
            } elseif ($UserRole == 6) {
                return redirect(url('/disposition_representative/buissness_accounts'))->with('message', 'Investor account has been deleted successfully');
            }
        } else {
            DB::rollback();
            if ($UserRole == 1) {
    ...
    public function AdminEditInvestor(Request $request)
    {
        $page = "investors";
        $Role = Session::get('user_role');
        $user_id = $request['id'];
        ...
        return view('admin.investors.edit-investor', compact('page', 'cities', 'counties', 'states', 'serving_locations', '_states', 'user_id', 'u_details', 'Role'));
    }

    public function AdminUpdateInvestor(Request $request)
    {
        $UserRole = Session::get('user_role');
        $user_id = $request['id'];
        $FirstName = ucwords(strtolower($request['firstname']));
        ...
        if ($OldEmail != $Email) {
            ...
            if ($check_user_email > 0) {
                if ($UserRole == 1) {
                    return redirect(url('/admin/investors'))->with('error', 'Error! Email already exists');
                } else {
                    return redirect(url('/global_manager/investors'))->with('error', 'Error! Email already exists');
                }
            }
        }
        ...
        DB::commit();
        if ($UserRole == 1) {
            return redirect(url('/admin/buissness_accounts'))->with('message', 'Investor record has been updated successfully');
        } elseif ($UserRole == 2) {
            return redirect(url('/global_manager/buissness_accounts'))->with('message', 'Investor record has been updated successfully');
        } elseif ($UserRole == 6) {
            return redirect(url('/disposition_representative/buissness_accounts'))->with('message', 'Investor record has been updated successfully');
        }
    }
}
